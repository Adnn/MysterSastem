...
#define PAIRING(X, Y, route)   \
    Register<value_8b> X()     { return makeRegister(mActiveRegisters.route.m##X##Y.store[0], #X); } \
    Register<value_8b> Y()     { return makeRegister(mActiveRegisters.route.m##X##Y.store[1], #Y); } \
...
struct RegisterSet
{
    ...
    Register<value_16b> identify16(opcode_t aSource)
    {
